#ifndef EMD_KERNEL_CU
#define EMD_KERNEL_CU
...
at::Tensor ApproxMatchForward(
  ...
  TORCH_CHECK(xyz2.size(0) == b, "xyz2 batch size must match xyz1");
  TORCH_CHECK(xyz1.size(2) == 3, "xyz1 must have 3 dimensions");
  TORCH_CHECK(xyz2.size(2) == 3, "xyz2 must have 3 dimensions");
  CHECK_INPUT(xyz1);
  CHECK_INPUT(xyz2);
